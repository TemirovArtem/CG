...
// Структура для хранения параметров рендер-объекта
struct RenderItem
{
	RenderItem() = default;
	RenderItem(const RenderItem& rhs) = delete;

	// Мировая матрица объекта (позиция, поворот, масштаб)
	XMFLOAT4X4 World = MathHelper::Identity4x4();
	XMFLOAT4X4 PrevWorld = MathHelper::Identity4x4(); // Предыдущая мировая матрица для motion vectors
	...
	// Флаг "грязных" данных - нужно обновить константный буфер
	// Устанавливаем = gNumFrameResources, чтобы обновить все frame resources
	int NumFramesDirty = gNumFrameResources;

	// Индекс в GPU константном буфере для этого объекта
	UINT ObjCBIndex = -1;

	Material* Mat = nullptr;
	Material* BaseMat = nullptr; // Оригинальный материал (для восстановления после отладочных режимов)
	MeshGeometry* Geo = nullptr;

	// Тип примитивов
	D3D12_PRIMITIVE_TOPOLOGY PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

	// Параметры для DrawIndexedInstanced
	UINT IndexCount = 0;
...
// Константы для TAA (Temporal Anti-Aliasing)
struct TAAConstants
{
	float Alpha = 0.1f;
...
public:
	...
	RayTracingApp(const RayTracingApp& rhs) = delete;
	RayTracingApp& operator=(const RayTracingApp& rhs) = delete;
	~RayTracingApp();
...
void RayTracingApp::MoveBackFwd(float step) {
	XMFLOAT3 newPos;
...
void RayTracingApp::MoveLeftRight(float step) {
	XMFLOAT3 newPos;
...
void RayTracingApp::MoveUpDown(float step) {
	XMFLOAT3 newPos;
...
void RayTracingApp::CreateSceneTexture()
{
	mSceneTexture.Reset();
...
{
	imguiID = 0;
	// Переключаемся на следующий frame resource
	mCurrFrameResourceIndex = (mCurrFrameResourceIndex + 1) % gNumFrameResources;
	mCurrFrameResource = mFrameResources[mCurrFrameResourceIndex].get();

	// Проверяем, завершил ли GPU обработку команд текущего frame resource
	// Если нет, ждем пока GPU завершит работу
	...
	// Пересоздаем DXR shadow mask если изменился downscale
...
void RayTracingApp::RotateSpotlightTowardCursor(int x, int y)
{
...
void RayTracingApp::OnKeyPressed(const GameTimer& gt, WPARAM key)
{
...
void RayTracingApp::OnKeyReleased(const GameTimer& gt, WPARAM key)
{
...
std::wstring RayTracingApp::GetCamSpeed()
{
	return std::to_wstring(cam.GetSpeed());
}

void RayTracingApp::UpdateCamera(const GameTimer& gt)
{
...
void RayTracingApp::AnimateMaterials(const GameTimer& gt)
{

}

void RayTracingApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void RayTracingApp::UpdateLightCBs(const GameTimer& gt)
{
	auto currLightCB = mCurrFrameResource->LightCB.get();
	auto currShadowCB = mCurrFrameResource->PassShadowCB.get();

	// Цикл день/ночь: анимация направленного света (солнца)
	if (mDayNightCycleEnabled && mDayNightCycleDuration > 0.01f)
	{
		...
		float sunY = sinf(angle);  // высота: 0 на горизонте, 1 в зените
		float sunX = cosf(angle);   // восток-запад
		XMFLOAT3 towardSun = { sunX, sunY, 0.0f };
		XMVECTOR v = XMLoadFloat3(&towardSun);
		v = XMVector3Normalize(v);
		XMFLOAT3 lightDir;
		XMStoreFloat3(&lightDir, -v); // направление света = от солнца к сцене
	...
	// Единое окно ImGui для всех источников света
	...
	for (auto& l : mLights)
	{
		...
		if (l.type == 0) // Ambient Light
		{
			...
			ImGui::Text("Ambient Light %d", lId);
			ImGui::DragFloat("Strength", (float*)&l.Strength, 0.02f, 0.0f, 10.0f);
			ImGui::Separator();
			ImGui::PopID();
		}
		else if (l.type == 1) // Point Light (обновляем без ImGui)
		{
	...
	// Синхронизация первого направленного света с атмосферой
	for (const auto& l : mLights)
		if (l.type == 2) {
...
void RayTracingApp::UpdateMaterialCBs(const GameTimer& gt)
{
	...
	for (auto& e : mMaterials)
	{
		...
		if (mat->NumFramesDirty > 0)
		{
			...
			// Следующий FrameResource тоже нужно обновить
			mat->NumFramesDirty--;
		}
	}
}

void RayTracingApp::UpdateMainPassCB(const GameTimer& gt)
{
	// View / Proj базовые
...
void RayTracingApp::CreateGBuffer()
{
	const DXGI_FORMAT positionFormat = DXGI_FORMAT_R32G32B32A32_FLOAT;
	const DXGI_FORMAT normalFormat = DXGI_FORMAT_R16G16B16A16_FLOAT;
	const DXGI_FORMAT albedoFormat = DXGI_FORMAT_R8G8B8A8_UNORM;
...
void RayTracingApp::LoadAllTextures()
{
	// Загружаем текстуры из корня Textures/
	...
	for (const auto& entry : std::filesystem::directory_iterator("Textures/textures"))
	{
		if (entry.is_regular_file() && entry.path().extension() == ".dds")
		{
			...
			filepath = filepath.substr(18, filepath.size());
	...
	tryLoad("textures/prefiltered");
	tryLoad("textures/irradiance");
	tryLoad("textures/brdfLUT");
	tryLoad("textures/skybox");

	LoadTerrainTextures();
}

void RayTracingApp::LoadTerrainTextures()
{
...
void RayTracingApp::LoadTexture(const std::string& name)
{
	...
	{
		std::cout << "[LoadTexture] File not found: " << name << "\n";
		return;
	}
	...
	{
		std::cout << "[LoadTexture] Failed to load: " << name << "\n";
		return;
	}
...
void RayTracingApp::BuildLightingRootSignature()
{
...
void RayTracingApp::BuildShadowPassRootSignature()
{
...
void RayTracingApp::BuildPostProcessRootSignature()
{
...
void RayTracingApp::BuildDxrShadowPSO()
{
	// Check DXR support.
...
void RayTracingApp::CreateSpotLight(XMFLOAT3 pos, XMFLOAT3 rot, XMFLOAT3 color, float faloff_start, float faloff_end, float strength, float spotpower)
{
	Light light = {};
	light.LightCBIndex = mLights.size();
...
void RayTracingApp::BuildLights()
{
...
void RayTracingApp::SetLightShapes()
{
	for (auto& light : mLights)
	{
...
void RayTracingApp::CreateMaterial(std::string _name, int _CBIndex, int _SRVDiffIndex, int _SRVNMapIndex, XMFLOAT4 _DiffuseAlbedo, XMFLOAT3 _FresnelR0, float _Roughness, float _Metallic)
{
...
void RayTracingApp::BuildShadowMapViews()
{
...
void RayTracingApp::BuildCustomMeshGeometry(std::string name, UINT& meshVertexOffset, UINT& meshIndexOffset, UINT& prevVertSize, UINT& prevIndSize, std::vector<Vertex>& vertices, std::vector<std::uint16_t>& indices, MeshGeometry* Geo)
{
	...
	{
		std::cerr << "Assimp error loading " << name << ": " << importer.GetErrorString() << std::endl;
		return; // Важно! Выходим из функции если модель не загрузилась
	}
	...
	for (int k = 0; k < scene->mNumMaterials; k++)
	{
		...
		// Sponza/mesh materials: high roughness (matte), no metallic to avoid wet look
...
void RayTracingApp::BuildShapeGeometry()
{
	...
	BuildCustomMeshGeometry("african_head", meshVertexOffset, meshIndexOffset, prevVertSize, prevIndSize, vertices, indices, geo.get());
...
void RayTracingApp::BuildTerrainGeometry()
{
	GeometryGenerator geoGen;
...
void RayTracingApp::BuildFrameResources()
{
	FlushCommandQueue();
...
void RayTracingApp::BuildMaterials()
{
	// Отладочный вывод - проверяем какие текстуры загружены
	std::cout << "\n=== BuildMaterials: Loaded textures ===" << std::endl;
	for (const auto& kv : mTextures)
	{
	...
	int bricksIdx = (TexOffsets.find("bricks") != TexOffsets.end()) ? TexOffsets["bricks"] : 0;
...
void RayTracingApp::BuildRenderItems()
{
	// Африканская голова
	RenderCustomMesh("head", "african_head", "HeadMat", XMFLOAT3(3, 3, 3), XMFLOAT3(0, 3.14, 0), XMFLOAT3(0, 3, 0));

	// Плоскость под головой для теней с текстурой bricks
	auto planeRitem = std::make_unique<RenderItem>();
	planeRitem->Name = "ground";
	XMStoreFloat4x4(&planeRitem->World, XMMatrixScaling(10.0f, 1.0f, 10.0f) * XMMatrixTranslation(0.0f, 0.0f, 0.0f));
	planeRitem->PrevWorld = planeRitem->World;
	XMStoreFloat4x4(&planeRitem->TexTransform, XMMatrixScaling(5, 5, 1)); // Повторяем текстуру
	planeRitem->ObjCBIndex = (UINT)mAllRitems.size();
	planeRitem->Mat = mMaterials["BricksMat"].get();
	planeRitem->BaseMat = planeRitem->Mat;
	planeRitem->Geo = mGeometries["shapeGeo"].get();
	planeRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	planeRitem->IndexCount = planeRitem->Geo->DrawArgs["grid"].IndexCount;
	planeRitem->StartIndexLocation = planeRitem->Geo->DrawArgs["grid"].StartIndexLocation;
...
void RayTracingApp::DrawSceneToShadowMap()
{
...
void RayTracingApp::DeferredDraw(const GameTimer& gt)
{
	auto cmdListAlloc = mCurrFrameResource->CmdListAlloc;
	ThrowIfFailed(cmdListAlloc->Reset());
...
void RayTracingApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	UINT objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(ObjectConstants));
...
void RayTracingApp::DrawTerrain(ID3D12GraphicsCommandList* cmdList)
{
...
void RayTracingApp::CreateDxrShadowMaskResources()
{
...
void RayTracingApp::CreateDxrShadowDescriptors()
{
...
void RayTracingApp::DispatchDxrShadowMask(ID3D12GraphicsCommandList* cmdList)
{
	if (!mEnableDxrShadows) return;
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> RayTracingApp::GetStaticSamplers()
{
...
void RayTracingApp::CreateTaaHistoryTextures()
{
	for (int i = 0; i < 2; ++i)
		mTaaHistory[i].Reset();
...
void RayTracingApp::CreateTaaHistoryRtvs()
{
	D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = {};
	rtvDesc.Format = mBackBufferFormat;
	rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;
...
void RayTracingApp::BuildTaaRootSignature()
{
...
void RayTracingApp::CreateTaaDepthHistoryTextures()
{
...
inline void RayTracingApp::Transition(
	ID3D12Resource* res,
