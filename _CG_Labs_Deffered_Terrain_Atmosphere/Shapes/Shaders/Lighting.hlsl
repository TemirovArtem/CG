...
// Compute atmospheric fog/scattering for objects
float3 ApplyAtmosphericScattering(float3 objectColor, float3 worldPos)
{
    if (gEnableAtmosphere == 0)
        return objectColor;
    ...
    // Atmospheric density based on distance (fog effect)
    // Scale distance to kilometers for atmospheric calculations
    float distanceKm = distance / 1000.0;
    
    // Optical depth increases with distance
    float opticalDepth = distanceKm * 0.01 * gAtmosphereDensity;
    opticalDepth = min(opticalDepth, 10.0); // Clamp to prevent over-darkening
    
    // Rayleigh and Mie scattering coefficients
    float3 betaR = gRayleighScattering * 1000.0; // Scale for scene units
    float3 betaM = gMieScattering * 1000.0 * gAtmosphereDensity;
    
    // Extinction (how much light is absorbed/scattered away)
    ...
    float3 inscatter = (betaR * phaseR + betaM * phaseM) * (1.0 - extinction);
    
    // Sun color contribution
    float3 sunColor = float3(1.0, 0.95, 0.9) * gSunIntensity * 0.05;
    inscatter *= sunColor;
    
    // Apply atmospheric perspective
    // Objects fade into atmospheric color with distance
    float3 atmosphericColor = inscatter;
    float3 finalColor = objectColor * extinction + atmosphericColor;
...
float4 PS(VSOutput pin) : SV_Target 
{
    ...
    color.rgb = ApplyAtmosphericScattering(color.rgb, posW);
