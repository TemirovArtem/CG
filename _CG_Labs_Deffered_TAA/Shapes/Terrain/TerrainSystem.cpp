...
namespace Terrain
{
                                    ...
                                    DXGI_FORMAT depthFormat)
    {
        ...
        // Инитим рендерер с правильными форматами G-Buffer
        mRenderer.Initialize(device, cmdList, albedoFormat, normalFormat, velocityFormat, depthFormat);
        ...
        // Порядок в куче - height, albedo, normal
    ...
    // Нужны форматы целевых буферов (альбедо, нормаль, velocity) и глубины
    //
    ...
    void TerrainSystem::BuildPSO(ID3D12Device* device,
                                  DXGI_FORMAT albedoFormat,
                                  DXGI_FORMAT normalFormat,
                                  DXGI_FORMAT velocityFormat,  // NEW: Velocity buffer format
                                  DXGI_FORMAT depthFormat)
    {
        mRenderer.BuildPSO(device, albedoFormat, normalFormat, velocityFormat, depthFormat);
    }
